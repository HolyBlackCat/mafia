...
# Checks if $1 is a directory, as opposed to a file or not existing at all.
override is_dir = $(if $(call safe_wildcard,$1/.),1)
...
# `-c`, plus either `--show-progress -q` for wget1 or `--progress=bar` for wget2.
...
WGET_FLAGS = $(if $(__wget_progress_flag),,$(call var,__wget_progress_flag := -c $(if $(filter 0,$(call shell_status,wget --version | head -1 | grep Wget2)),--progress=bar,--show-progress -q)))$(__wget_progress_flag)

# Downloads url $1 to file $2. Expands to nothing. Triggers an error on failure.
...
# Given archive filename or path $1 (that may not exist), tries to determine the archive type. Returns one of `archive_types`, or empty on failure.
override archive_classify_filename = $(firstword $(foreach x,$(archive_types),$(if $(call archive_is-$x,$(notdir $1)),$x)))
...
# `-o` allows us to overwrite files. Good for at least `make -B deps/archives`.
...
# Library source archives are looked up relative to this.
LIB_SRC_DIR := $(proj_dir)/deps/archives
...
# `make dist-deps` creates an archive with all downloaded dependencies with this name.
DIST_DEPS_ARCHIVE := $(proj_dir)/deps.zip
...
# Downloads the source archive. If there's no URL, does nothing (or complains if the file is missing).
...
.PHONY: clean-downloaded-lib-$(__lib_name)
$(if $(__has_url),clean-downloaded-lib-$(__lib_name): ; @rm -f $(call quote,$(__ar_path)))
